...
) ENGINE=InnoDB AUTO_INCREMENT=6755 DEFAULT CHARSET=utf8mb4
...
/*!50003 CREATE */ /*!50017 DEFINER = 'root'@'localhost' */ /*!50003 TRIGGER `trg_inventory_after_update_audit` AFTER UPDATE ON `inventory` FOR EACH ROW BEGIN
...
/*!50003 CREATE */ /*!50017 DEFINER = 'root'@'localhost' */ /*!50003 TRIGGER `trg_order_items_after_insert_audit` AFTER INSERT ON `order_items` FOR EACH ROW BEGIN
  INSERT INTO `stock_audit` (
    `item_id`,
    `action_type`,
    `action_date`,
  ...
  SELECT
    ...
    COALESCE(SUM(i.`quantity_on_hand`), 0),
    CONCAT('Order ', NEW.`order_id`, ' item added with quantity ', NEW.`quantity`, ' at branch ', o.`branch_location`)
  FROM `products` p
