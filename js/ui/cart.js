...
export function addToCart(productId) {
    ...
    if (currentQuantity >= availableStock) {
        showCartFeedback(`Only ${availableStock} item${availableStock === 1 ? '' : 's'} available.`);
        return;
    }
...
export function renderCartPage() {
    ...
    const shipping = subtotal > CHECKOUT_CONFIG.freeShippingThreshold ? 0 : CHECKOUT_CONFIG.shippingFee;
                ...
                <div class="order-summary">
                    ...
                    ${subtotal < CHECKOUT_CONFIG.freeShippingThreshold ? `
                        <div style="background: #dbeafe; color: #1e40af; padding: 0.75rem; border-radius: 0.375rem; font-size: 0.875rem; margin: 1rem 0;">
                            Add ${formatPrice(CHECKOUT_CONFIG.freeShippingThreshold - subtotal)} more for free shipping!
                        </div>
                    ` : ''}
